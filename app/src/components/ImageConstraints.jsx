          ...
          <div className="m3-card">
            ...
            <div className="m3-field-group">
              <div className="m3-text-field">
                ...
                {constraints.backendType === "vulkan" && backendOptions?.vulkanDevices?.length > 0 && (
                  <div style={{ marginTop: "16px" }}>
                    <label className="m3-text-field-label">GPU Device</label>
                    <select
                      value={constraints.vulkanDevice ?? "auto"}
                      onChange={(e) => updateConstraint("vulkanDevice", e.target.value)}
                      style={{
                        width: "100%",
                        marginTop: "6px",
                        padding: "10px 12px",
                    ...
                    >
                      <option value="auto">
                        Auto (recommended{(() => {
                          const rec = backendOptions.vulkanDevices.find((d) => d.index === backendOptions.recommendedVulkanIndex);
                          return rec ? ` — ${rec.name}` : "";
                        })()})
                      </option>
                      {backendOptions.vulkanDevices.map((d) => (
                        <option key={d.index} value={String(d.index)}>
                          {d.name}{d.type === "discrete" ? " — discrete" : d.type === "cpu" ? " — software (slow)" : " — integrated"}{d.matrixCores ? ", matrix cores" : ""}
                        </option>
                      ))}
                    </select>
                    <span style={{ fontSize: "0.75rem", color: "var(--md-sys-color-outline)", marginTop: "4px", lineHeight: 1.35, display: "block" }}>
                      Choose which GPU runs generation. Useful on machines with more than one GPU (e.g. a discrete card plus an integrated one). Auto prefers a dedicated card with matrix-core acceleration.
                    </span>
                  </div>
                )}
              </div>
